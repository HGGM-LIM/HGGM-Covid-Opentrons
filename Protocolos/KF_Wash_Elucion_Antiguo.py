...
def run(ctx: protocol_api.ProtocolContext):
    ...
    wash2_buffer_rack = robot.load_labware('nest_12_reservoir_15ml', '4',
        'nest_12_reservoir_15ml')    


    wash1_dest_rack = [robot.load_labware('nest_96_wellplate_2ml_deep', slot, 
        'source tuberack ') for slot in ['11','5']
    ]

    wash2_dest_rack = [robot.load_labware('nest_96_wellplate_2ml_deep',  slot, 
        'source tuberack ') for slot in ['7','1']
    ]
    ...
    def step1():
        ...
        for col, wash_dest_rack in enumerate(wash1_dest_rack):

            dest_wells = [well for well in wash_dest_rack.rows()[0]]
        
            list_dest = list(divide_destinations(dest_wells,4))

            for i, dest in enumerate(list_dest):

                wash_src = wash1_elution_buffer_rack.columns()[i+(3*col)][0]
    ...
    def step2():
        ...
        for col, wash_dest_rack in enumerate(wash2_dest_rack):

            dest_wells = [well for well in wash_dest_rack.rows()[0]]
            
            list_dest = list(divide_destinations(dest_wells,3))

            for i, dest in enumerate(list_dest):

                wash_src = wash2_buffer_rack.columns()[i+(4*col)][0]
