...
def run(ctx: protocol_api.ProtocolContext):
    ...
    # Labware
    # -----------------------------------------------------
    src_rack = robot.load_labware('opentrons_6_tuberack_falcon_50ml_conical', '5',
    ...
    dest_rack = [
        robot.load_labware(
            ...
            'dest tuberack')
        for slot in ['10', '9', '8', '7', '6', '4', '3', '2', '1']
    ]

    # -----------------------------------------------------
    # Reagens
    # -----------------------------------------------------
    leche_reagent = Reagent(name = 'Leche',
    ...
    # Tubes
    # -----------------------------------------------------
    leche_tube = Tube(name = 'Falcon 50mL Conical Centrifuge Tubes',
                max_volume = 50000,
                actual_volume = 40000,
    ...
    def step1():
        ...
        src_list=src_rack.wells()[0:4]
        dest_list = [p for r in dest_rack for w in r.rows() for p in w]

        list_dest = list(divide_destinations(dest_list,48))

        for i in range(len(src_list)):
            distribute_custom(pip = p1000,
                        reagent = leche_reagent,
                        tube_type = leche_tube,
                        volume = 1000,
                        src =src_list[i],
                        dest = list_dest[i],
                        disp_height = 30,
            ...
            leche_tube.actual_volume=40000
    ...
    # -----------------------------------------------------
    STEPS = {
        1:{'Execute': True,  'Function': step1, 'Description': 'Transfer leche'}
    }
