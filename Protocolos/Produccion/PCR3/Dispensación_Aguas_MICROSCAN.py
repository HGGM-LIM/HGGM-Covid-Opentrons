...
# #####################################################
metadata = {
    'protocolName': 'Dispensacón Aguas MICROSCAN',
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    dest_rack = [
        robot.load_labware(
            'tubovidriotapondorado_15_tuberack_15000ul', slot,
            'dest tuberack')
        for slot in ['10', '11', '8', '4','6', '3', '2', '1']
    ]

    # -----------------------------------------------------
    # Reagens
    # -----------------------------------------------------
    src_reagent = Reagent(name = 'Water',
                    flow_rate_aspirate = 20000,
                    flow_rate_dispense = 20000,
                    flow_rate_aspirate_mix = 20000,
                    flow_rate_dispense_mix = 20000)
    ...
    def step1():
        ...
        src_list = [p for r in src_rack for w in r.rows() for p in w]
        dest_list = [p for r in dest_rack for w in r.rows() for p in w]
        list_dest = list(divide_destinations(dest_list,10))
        list_dest=list_dest[0:12]
