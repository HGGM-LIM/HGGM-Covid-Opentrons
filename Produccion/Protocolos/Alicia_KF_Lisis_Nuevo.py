#ALICIA AREVALO GARCIA // 10/06/2020
# Protocolo King Fisher Nuevo
# Dispensamos Proteinasa-K y buffer de lisis.
# El protocolo sigue tres steps:
#1. La protk se encuentra en la A1 del soporte, con un volumen total de 1ml. Con la p20, dispensamos 10ul de protK en cada uno de los 96 pocillos de la placa "KingFisher Deep-well 96 plate" (func distribution_custom). 
#2 y 3. El Lisis estará en dos falcon de 50ml en las posiciones A3 y B3 de la gradilla, cada uno con un volumen total de 30ml. Con la p1000, dispensamos 550ul de lisis en cada pocillo de la placa "KingFisher Deep-well 96 plate" (func distribute_custom). La placa tiene 12 columnas. Se llenarán las seis primeras con el primer Falcon (step2) y las restantes con el segundo (step3). Después de cada columna (8 pocillos), se mezcla la lisis (func custom_mix).
...
# Protocol parameters
# #####################################################
NUM_SAMPLES = 96 #número de pocillos
RESET_TIPCOUNT = False #reiniciamos el conteo de puntas. false=recupera el valor anterior, y si hemos cancelado el proceso toma el contador no actualiza el nuevo valor
PROTOCOL_ID = "GM"
...
def distribute_custom(pip, reagent, tube_type, volume, src, dest, 
    extra_dispensal=0, disp_height=0, touch_tip_aspirate=False, touch_tip_dispense = False):
        
        max_trans_per_asp = (pip.max_volume - extra_dispensal) // volume
        ...
        for i in range(len(list_dest)):
            ...
            if touch_tip_aspirate:
                    #pip.touch_tip(radius=0.6,v_offset=-11,speed=70.0)

                    pip.touch_tip(radius=1.0,v_offset=-5,speed=20.0)
...
def run(ctx: protocol_api.ProtocolContext):
    ...
    def step1():
        ...
        list_dest = list(divide_destinations(dest_rack.wells(),16))
    ...
    # -----------------------------------------------------
    STEPS = {
        1:{'Execute': False,  'Function': step1, 'Description': 'Transfer Proteinasa K'},
